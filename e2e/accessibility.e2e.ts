...
test.describe('Keyboard Accessibility', () => {
  ...
  test.describe('Tab Order Navigation', () => {
    test('Tab order follows logical reading order in main view', async ({ page }) => {
      ...
      const expectedOrder = ['Decks', 'Cards', 'Toggle Tray', 'Settings', 'Dictionary', 'Create Card', 'Start Review'];
    ...
    test('All interactive elements in card list are reachable via keyboard', async ({ page }) => {
      ...
      const interactiveCount = focusedElements.filter(el => el.includes('button') || el.includes('input') || el.includes('select') || el.includes('textarea')).length;
      expect(interactiveCount).toBeGreaterThan(0);
      fs.writeFileSync(path.join(evidenceDir, 'task-16-cards-tab-order.txt'), 'Cards view tab navigation:\n' + focusedElements.map((el, i) => (i + 1) + '. ' + el).join('\n') + '\n\nResult: ' + interactiveCount + ' interactive elements found and reachable via keyboard.');
    });
  });
  ...
  test.describe('Focus Indicator Visibility', () => {
    test('Focus indicator is visible on focused buttons', async ({ page }) => {
      ...
      const focusStyles = await page.evaluate(() => {
        ...
        return { outline: styles.outline, outlineWidth: styles.outlineWidth, outlineColor: styles.outlineColor, outlineStyle: styles.outlineStyle, outlineOffset: styles.outlineOffset, boxShadow: styles.boxShadow };
      });
      const hasVisibleFocus = focusStyles && ((focusStyles.outlineStyle && focusStyles.outlineStyle !== 'none') || (focusStyles.boxShadow && focusStyles.boxShadow !== 'none'));
      expect(hasVisibleFocus).toBe(true);
      await page.screenshot({ path: path.join(evidenceDir, 'task-16-focus-indicator.png') });
      fs.writeFileSync(path.join(evidenceDir, 'task-16-focus-styles.txt'), 'Focus indicator styles on Decks button:\n' + JSON.stringify(focusStyles, null, 2) + '\n\nHas visible focus: ' + hasVisibleFocus + '\n\nResult: Focus indicator is visible when element is keyboard focused.');
    });
    ...
    test('Focus indicator is visible on inputs', async ({ page }) => {
      ...
      const focusStyles = await page.evaluate(() => {
        ...
        if (!active) return null;
        const styles = window.getComputedStyle(active);
        return { outline: styles.outline, outlineWidth: styles.outlineWidth, outlineColor: styles.outlineColor, outlineStyle: styles.outlineStyle, outlineOffset: styles.outlineOffset, boxShadow: styles.boxShadow, ringWidth: styles.getPropertyValue('--tw-ring-width') };
      });
      const hasVisibleFocus = focusStyles && ((focusStyles.outlineStyle && focusStyles.outlineStyle !== 'none') || (focusStyles.boxShadow && focusStyles.boxShadow !== 'none') || (focusStyles.ringWidth && focusStyles.ringWidth !== '0px'));
      expect(hasVisibleFocus).toBe(true);
      await page.screenshot({ path: path.join(evidenceDir, 'task-16-input-focus.png') });
      fs.writeFileSync(path.join(evidenceDir, 'task-16-input-focus-styles.txt'), 'Focus indicator styles on Front input:\n' + JSON.stringify(focusStyles, null, 2) + '\n\nHas visible focus: ' + hasVisibleFocus + '\n\nResult: Input elements have visible focus indicators when keyboard focused.');
  ...
  test.describe('Keyboard Shortcuts', () => {
    test('Cmd+, opens Settings from any focus state', async ({ page }) => {
      const focusStates = ['body' as const, 'Decks button', 'Cards button', 'Start Review button'];
      ...
      fs.writeFileSync(path.join(evidenceDir, 'task-16-cmd-comma-shortcut.txt'), 'Cmd+, keyboard shortcut test from various focus states:\n\n' + results.join('\n') + '\n\nResult: Keyboard shortcut works from all tested focus states.');
    });

    test('Cmd+N opens Create Card from any focus state', async ({ page }) => {
      const focusStates = ['body' as const, 'Decks button', 'Cards button', 'Settings button'];
      const results: string[] = [];
      for (const state of focusStates) {
        await page.getByRole('button', { name: 'Decks' }).click();
        if (state === 'body') {
      ...
      fs.writeFileSync(path.join(evidenceDir, 'task-16-cmd-n-shortcut.txt'), 'Cmd+N keyboard shortcut test from various focus states:\n\n' + results.join('\n') + '\n\nResult: Keyboard shortcut works from all tested focus states.');
    });
  ...
  test.describe('Modal Focus Trap', () => {
    test('Settings modal traps focus within dialog', async ({ page }) => {
      ...
      const wrappedFocused = await page.evaluate(() => {
        ...
        const text = active.textContent?.trim().substring(0, 30);
        return { tagName: active.tagName.toLowerCase(), inModal: !!parentDialog, text: text };
      });
    ...
    test('Card Form modal traps focus within form', async ({ page }) => {
      ...
      const lastFocused = await page.evaluate(() => {
        const active = document.activeElement;
        if (!active) return null;
        return { tagName: active.tagName.toLowerCase(), text: active.textContent?.trim().substring(0, 30) };
      });
      await page.keyboard.press('Tab');
      const wrappedFocused = await page.evaluate(() => {
        const active = document.activeElement;
        if (!active) return null;
        return { tagName: active.tagName.toLowerCase(), isInput: active.tagName === 'INPUT' || active.tagName === 'TEXTAREA' || active.tagName === 'SELECT' };
      });
      expect(wrappedFocused?.isInput).toBe(true);
      fs.writeFileSync(path.join(evidenceDir, 'task-16-cardform-focus-trap.txt'), 'Card Form modal focus trap test:\n' + '- Total tabbable elements: ' + elementCount + '\n' + '- Last focused: ' + lastFocused?.tagName + ' (' + lastFocused?.text + ')\n' + '- After Tab wrap: ' + wrappedFocused?.tagName + ' (is input: ' + wrappedFocused?.isInput + ')\n\n' + 'Result: Focus is properly trapped within the Card Form modal.');
      await page.keyboard.press('Escape');
    });
  });
